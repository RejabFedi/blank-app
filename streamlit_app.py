...
# --- CSS personnalisé amélioré pour la réactivité ---
st.markdown("""
<style>
    .main-header {
        font-size: 3rem;
    ...
    /* Kanban grid layout for large screens */
    .kanban-container {
        display: none; /* Hide the full Kanban on small screens by default */
    }
    .kanban-grid {
        display: grid;
    ...
    .kanban-column {
        min-width: 150px; /* Ensures columns don't get too narrow */
    }
    
    /* Responsive layout for smaller screens */
    @media (min-width: 768px) {
        .kanban-container {
            display: block; /* Show the full Kanban on large screens */
        }
        .kanban-mobile {
            display: none; /* Hide the mobile list on large screens */
        }
    }
    
    /* Mobile-first approach for smaller screens */
    .kanban-mobile {
        display: block; /* Show the mobile list on small screens by default */
    }
    .status-section {
    ...
    .task-card {
        ...
        margin-bottom: 1rem; /* Add spacing between cards in the mobile view */
    }
    .task-card:hover {
...
# --- Display tasks based on screen size (responsive part) ---
statuses = ["À faire", "En cours", "En revue", "Approuvé", "Rejeté", "Terminé", "Archivé"]

# Full Kanban for large screens
st.markdown('<div class="kanban-container">', unsafe_allow_html=True)
st.markdown('<div class="kanban-grid">', unsafe_allow_html=True)
    ...
    if not tasks_status.empty:
        for index, task in tasks_status.iterrows():
            with st.expander(f"**{task['Tâche']}**"):
                ...
                st.write(f"✅ **Confirmé** : {'Oui' if task['Confirmé'] == 'Oui' else 'Non'}")
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("✏️ Modifier", key=f"edit_{task['id']}"):
                        st.session_state[f"edit_modal_{task['id']}"] = True
                with col2:
                    if st.button("🗑️ Supprimer", key=f"delete_{task['id']}"):
                        st.session_state[f"delete_modal_{task['id']}"] = True
    else:
...
st.markdown('<div class="kanban-mobile">', unsafe_allow_html=True)
for status in statuses:
    st.markdown(f'<div class="status-section"><h3>{status}</h3>', unsafe_allow_html=True)
    tasks_status = filtered_df[filtered_df["Statut"] == status]
    if not tasks_status.empty:
        for index, task in tasks_status.iterrows():
            with st.expander(f"**{task['Tâche']}**"):
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("✏️ Modifier", key=f"edit_mobile_{task['id']}"):
                        st.session_state[f"edit_modal_{task['id']}"] = True
                with col2:
                    if st.button("🗑️ Supprimer", key=f"delete_mobile_{task['id']}"):
                        st.session_state[f"delete_modal_{task['id']}"] = True
    else:
        st.info("Aucune tâche")
    st.markdown('</div>', unsafe_allow_html=True)
...
# Modals for editing and deleting (unchanged logic)
for index, task in filtered_df.iterrows():
    # Modification Modal
    if f"edit_modal_{task['id']}" in st.session_state and st.session_state[f"edit_modal_{task['id']}"]:
        with st.form(f"edit_form_{task['id']}"):
            st.subheader("Modifier la tâche")
            edit_tache = st.text_input("Tâche", value=task["Tâche"], key=f"edit_tache_{task['id']}")
            edit_responsable = st.selectbox("Responsable", ["Fedi", "Chayma", "Alaa", "Amen", "Wafa"],
                                            ...
                                            key=f"edit_responsable_{task['id']}")
            current_date = task["Date limite"] if "Date limite" in task and pd.notna(task["Date limite"]) else datetime.today().date()
            edit_date_limite = st.date_input("Date limite", value=current_date, key=f"edit_date_{task['id']}")
            edit_statut = st.selectbox("Statut", ["À faire", "En cours", "En revue", "Approuvé", "Rejeté", "Terminé", "Archivé"],
                                       ...
                                       key=f"edit_statut_{task['id']}")
            edit_confirme = st.checkbox("Confirmé ?", value=task["Confirmé"] == "Oui", key=f"edit_confirm_{task['id']}")
            col1, col2 = st.columns(2)
            with col1:
                if st.form_submit_button("Confirmer la modification", type="primary"):
                    if edit_tache.strip() == "":
                        ...
                        with st.spinner("Modification en cours..."):
                            updated_task = {
                                "Tâche": edit_tache, "Responsable": edit_responsable, "Date limite": str(edit_date_limite),
                                "Statut": edit_statut, "Confirmé": "Oui" if edit_confirme else "Non"
                            }
                            success, message = update_task(task['id'], updated_task)
                            if success:
                                st.success("✅ " + message)
                                st.session_state[f"edit_modal_{task['id']}"] = False
                                st.cache_data.clear()
                                st.rerun()
                            else:
                                st.error("❌ " + message)
    ...
    # Deletion Modal
    if f"delete_modal_{task['id']}" in st.session_state and st.session_state[f"delete_modal_{task['id']}"]:
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("Oui, supprimer", type="primary", key=f"confirm_delete_{task['id']}"):
                with st.spinner("Suppression en cours..."):
                    success, message = delete_task(task['id'])
                    if success:
                        st.success("✅ " + message)
                        st.session_state[f"delete_modal_{task['id']}"] = False
                        st.cache_data.clear()
                        st.rerun()
                    else:
                        st.error("❌ " + message)
        with col2:
            if st.button("Annuler", key=f"cancel_delete_{task['id']}"):
                st.session_state[f"delete_modal_{task['id']}"] = False
                st.rerun()
    ...
    if submitted:
        if tache.strip() == "":
            ...
            with st.spinner("Ajout en cours..."):
                time.sleep(1)
                new_task = {
                    "Tâche": tache, "Responsable": responsable, "Date limite": str(date_limite),
                    "Statut": statut, "Confirmé": "Oui" if confirme else "Non"
                }
